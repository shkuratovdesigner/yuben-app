...
<body><div class="wrap">
  ...
  <p class="foot">Generated by YuBen · ${esc(new Date().toISOString().slice(0, 10))} · ${
    result.meta.is_example
      ? `Sample data — this export came from the bundled example run, not research you
  ran. Video IDs, titles and channels are real; counts are representative values.`
      : `Every link and number
  is re-verified from the YouTube Data API — narrative is AI-authored, facts are not.`
  }</p>
</div></body></html>`
}
